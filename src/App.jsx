...
async function sbRequestLoginCode(email) {
  // Check the users table (not auth_bridge) so accounts created via magic-link also work
  const exists = await sbCheckUserExists(email);
  if (!exists) throw new Error("No account found for this email. Sign up first.");

  const code = generateCode();
  const newPassword = generatePassword();
  const expiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();

  // Upsert: creates a fresh auth_bridge row if missing (e.g. old magic-link accounts),
  // or updates the existing row with a new code + fresh password.
  // We always generate a fresh password so we can immediately update Supabase Auth
  // after the code is verified -- this self-heals any password mismatch.
  const { error } = await supabase.from("auth_bridge").upsert({
    email,
...
async function sbVerifyCode(email, codeInput) {
  ...
  if (bridge.is_new_signup) {
    // First-time signup: create the Supabase Auth user with the stored password
    ...
    // If no session yet (confirm email still on), sign in explicitly
    if (!data.session && data.user) {
      const { data: d2, error: e2 } = await supabase.auth.signInWithPassword({
        email, password: bridge.current_password,
      });
      if (e2) throw new Error("Account created but could not sign in. Please try logging in.");
      return d2.user;
    }
    return data.user;
  } else {
    // Returning user: use our API endpoint (which has the Supabase service role key)
    // to set a known password, then sign in with it. This handles both:
    // - Accounts created via our EmailJS flow (password in auth_bridge matches)
    // - Legacy accounts created via magic-link (password unknown, needs reset via admin API)
    const resetResp = await fetch("https://myrecipecards-api.vercel.app/api/reset-password", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, password: bridge.current_password }),
    });
    if (!resetResp.ok) {
      const err = await resetResp.json();
      throw new Error(err.error || "Could not reset password. Please try again.");
    }
          ...
          return (
            <div key={dk+ci}
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:1}}>
                ...
                {thisMonth&&hasMeals&&(
                  {!isMobile&&<button title={isGrocerySelected?"Remove from grocery list":"Add to grocery list"}
  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:T.bg}}>
        {/* Header */}
        <header style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:isMobile?"0 12px":"0 18px",position:"sticky",top:0,zIndex:30}}>
          <div style={{maxWidth:1100,margin:"0 auto"}}>
            {/* Top row: logo + action buttons */}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",height:52}}>
              <h1 style={{fontFamily:serif,fontSize:isMobile?16:19,fontWeight:600,marginRight:isMobile?8:18,flexShrink:0}}>
                <span style={{color:T.muted}}>my</span><span style={{color:T.terra}}>recipe</span><span style={{color:T.sage}}>cards</span>
              </h1>
              {!isMobile&&<nav style={{display:"flex",flex:1}}>
              ...
              <div style={{display:"flex",gap:isMobile?4:6,alignItems:"center"}}>
                {inbox.length>0&&(
                  <button className="bg" onClick={()=>setInboxOpen(o=>!o)} style={{position:"relative",padding:"6px 8px",fontSize:12}}>
                    🔔{isMobile?` ${inbox.length}`:<> {inbox.length}</>}
                ...
                {!isMobile&&<button className="bg" onClick={()=>setShowSettings(true)} style={{position:"relative",padding:"6px 11px",fontSize:12}}>
                  ⚙ Settings
                  {hasSettings&&<span style={{width:5,height:5,borderRadius:"50%",background:T.terra,position:"absolute",top:3,right:3}}/>}
                </button>}
                <button className="bp" onClick={()=>setShowAdd(true)} style={{padding:"6px 10px",fontSize:isMobile?12:13}}>{isMobile?"+ Add":"+ Add recipe"}</button>
                {!isMobile&&<button className="bg" onClick={logout} style={{fontSize:12,padding:"6px 10px"}}>Log out</button>}
                {isMobile&&<button className="bg" onClick={()=>setShowSettings(true)} style={{position:"relative",padding:"6px 8px",fontSize:14}}>
                  ⚙
            ...
            {isMobile&&<nav style={{display:"flex",borderTop:`1px solid ${T.border}`}}>
              <button className={`nav-btn${page==="recipes"?" active":""}`} onClick={()=>setPage("recipes")} style={{flex:1,textAlign:"center",padding:"8px 0",fontSize:13}}>Recipes</button>
              <button className={`nav-btn${page==="mealplan"?" active":""}`} onClick={()=>setPage("mealplan")} style={{flex:1,textAlign:"center",padding:"8px 0",fontSize:13}}>Meal Plan</button>
              <button className="bg" onClick={logout} style={{fontSize:12,padding:"6px 12px",border:"none",borderLeft:`1px solid ${T.border}`,borderRadius:0}}>Log out</button>
            </nav>}
          </div>
        </header>
        ...
        {/* Recipes Page */}
        {page==="recipes"&&(
          <main style={{maxWidth:1100,margin:"0 auto",padding:isMobile?"14px 10px":"20px 18px"}}>
