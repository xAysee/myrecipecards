  ...
  async function requestCode() {
    ...
    try {
      const resp = await fetch("https://api.emailjs.com/api/v1.0/email/send", {
        ...
        body: JSON.stringify({
          service_id: "service_lv9leie",
          template_id: "template_h4v09jw",
          user_id: "Azaqm6NZVZ1NwOklh",
          template_params: {
            to_email: key,
            to_name: name.trim() || key,
            code,
          },
        }),
      });
...
export default function App() {
  ...
  // Uses Gemini (via gathered-api serverless function) for recipe extraction
  async function runImport(action) {
    ...
    setShowAdd(true);
    let parts = null; // Gemini "parts" array for the user turn
    ...
    try {
      if (action.type === "url") {
        ...
        if (isUrl) {
          ...
          try {
            ...
            parts = [{ text: "Extract the recipe from this page content. Return ONLY the JSON, nothing else.\n\n" + pageText }];
          } catch(_) {
            parts = [{ text: "Extract the recipe from this URL. You may know this site \u2014 return the recipe as JSON.\n\nURL: " + text }];
          }
        } else {
        ...
        parts = [
          { inline_data: { mime_type: "image/jpeg", data: action.imgSrc.split(",")[1] } },
          { text: "Extract the recipe from this photo." }
        ];
      } else if (action.type === "parts") {
        // action.parts came from Claude-style {type, source/text} blocks — convert to Gemini shape
        capturedImage = action.imgSrc || null;
        parts = action.parts.map(p => {
      ...
      // Step 2: call Gemini via our serverless proxy
      setImportJob(j=>({...(j||{}), loading:true, step:"Extracting", label:"Sending to Gemini\u2026", parsed:null, error:"", capturedImage}));
      const res = await fetch("https://gathered-api.vercel.app/api/extract-recipe", {
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body: JSON.stringify({
      ...
      const d = await res.json();
      if (d.error) {
        const m = d.error.message || JSON.stringify(d.error);
        throw new Error(m);
      }
