...
async function sbVerifyCode(email, codeInput) {
  ...
  if (bridge.is_new_signup) {
    // First-time verification: create the Supabase Auth user now that code is confirmed
    ...
    // If Supabase "Confirm email" is ON, data.user exists but data.session is null.
    // In that case, immediately sign in with the stored password to get a real session.
    if (!data.session && data.user) {
      const { data: signInData, error: signInError } = await supabase.auth.signInWithPassword({
        email, password: bridge.current_password,
      });
      if (signInError) throw new Error("Account created but login failed. Please disable \"Confirm email\" in Supabase Authentication settings.");
    ...
    if (error) {
      if (error.message?.toLowerCase().includes("invalid login credentials") ||
        ...
        throw new Error("Login failed — your account may not be confirmed yet. In your Supabase project go to Authentication → Configuration and disable \"Confirm email\", then try again.");
      }
      throw error;
    }
    await supabase.from("auth_bridge").update({
    ...
    <div className="card fi" onClick={()=>onClick(recipe)}
      ...
      <div style={{padding:"12px 14px",flex:1,display:"flex",flexDirection:"column",background:T.surface}}>
        <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:6}}>
          {recipe.tags.slice(0,3).map((t,i)=><span key={t} style={{display:"inline-block",fontSize:11,fontWeight:500,letterSpacing:".04em",textTransform:"uppercase",padding:"3px 9px",borderRadius:20,background:i%2?T.terraLight:T.sageLight,color:i%2?T.terra:T.sage}}>{t}</span>)}
        </div>
        ...
        <div style={{padding:"16px 20px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
          <div style={{flex:1,minWidth:0}}>
            {editing
              ...
              : <><div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:7}}>{draft.tags.map((t,i)=><span key={t} style={{display:"inline-block",fontSize:11,fontWeight:500,letterSpacing:".04em",textTransform:"uppercase",padding:"3px 9px",borderRadius:20,background:i%2?T.terraLight:T.sageLight,color:i%2?T.terra:T.sage}}>{t}</span>)}</div>
        ...
        <div style={{padding:"14px 18px"}}>
          ...
          {parsed&&<div className="fi">
            ...
            <div style={{padding:"11px 13px",background:T.paper,borderRadius:8,border:`1px solid ${T.border}`,marginBottom:11}}>
              <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:4}}>{(parsed.tags||[]).map((t,i)=><span key={t} style={{display:"inline-block",fontSize:11,fontWeight:500,letterSpacing:".04em",textTransform:"uppercase",padding:"3px 9px",borderRadius:20,background:i%2?T.terraLight:T.sageLight,color:i%2?T.terra:T.sage}}>{t}</span>)}</div>
          ...
          <main style={{maxWidth:1100,margin:"0 auto",padding:"20px 18px"}}>
            ...
            {!search&&!filters.length&&(
              <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:16}}>
                {allTags().map(t=><button key={t} onClick={()=>addFilter(t)} style={{cursor:"pointer",border:"none",fontFamily:sans,display:"inline-block",fontSize:11,fontWeight:500,letterSpacing:".04em",textTransform:"uppercase",padding:"3px 9px",borderRadius:20,background:T.sageLight,color:T.sage}}>{t}</button>)}
              </div>
            )}
