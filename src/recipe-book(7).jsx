...
function AuthScreen({ T, onLogin }) {
  ...
  async function verifyCode() {
    ...
    const key = email.trim().toLowerCase();
    try {
      const authUser = await sbVerifyOtp(key, codeInput.trim());
...
export default function App() {
  ...
  // ─── Bootstrap: check for an existing Supabase session on mount ─────────────
  useEffect(() => {
    (async () => {
    ...
    // Keep in sync if the session changes (e.g. token refresh, sign-out elsewhere)
