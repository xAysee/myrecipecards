...
async function sbSignUp(email, name) {
...
.nav-btn{background:transparent;border:none;padding:8px 16px;font-size:14px;color:${T.ink};border-bottom:2px solid transparent;border-radius:0;font-weight:500}
...
function AuthScreen({ T, onLogin }) {
  ...
  async function resend() {
    setResending(true);
    try {
      if (mode === "signup") await sbSignUp(email.trim().toLowerCase(), name.trim());
      else await sbSignIn(email.trim().toLowerCase());
      ...
      <div className="card fi" style={{width:"100%",maxWidth:400}}>
        ...
        <div style={{padding:"16px 28px 28px",display:"flex",flexDirection:"column",gap:11}}>
          {step==="email"&&<>
            ...
            <button className="bp" onClick={sendMagicLink} disabled={loading} style={{width:"100%",marginTop:2}}>
              {loading?<Dots/>:(mode==="login"?"Send login link":"Create account")}
            </button>
          </>}
          {step==="sent"&&<>
            <div style={{padding:"14px 16px",background:T.paper,borderRadius:8,textAlign:"center"}}>
              <div style={{fontSize:28,marginBottom:8}}>📬</div>
              <p style={{fontSize:14,color:T.ink,fontWeight:500,marginBottom:6}}>Check your email</p>
              <p style={{fontSize:13,color:T.muted,lineHeight:1.5}}>
                We sent a sign-in link to <strong>{email.trim()}</strong>. Open it on this device to finish {mode==="signup"?"creating your account":"logging in"}.
              </p>
            </div>
            <button className="bg" onClick={resend} disabled={resending} style={{width:"100%",fontSize:13}}>
              {resending?<Dots/>:"Resend email"}
            </button>
            <button className="bg" style={{width:"100%",fontSize:12}} onClick={()=>{setStep("email");setErr("");}}>
...
function RecipeCard({ recipe, onClick, T }) {
  return (
    <div className="card fi" onClick={()=>onClick(recipe)}
      style={{cursor:"pointer",transition:"transform .15s,box-shadow .15s",display:"flex",flexDirection:"column",height:"100%"}}
...
export default function App() {
  ...
  // ─── Bootstrap: detect session from a normal page load OR a magic-link redirect ─
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      // SIGNED_IN fires after a magic-link redirect establishes a new session.
      if (event === "INITIAL_SESSION" || event === "SIGNED_IN") {
        if (session?.user) {
          try {
            const userRow = await sbEnsureUserRow(session.user);
            await loadAllUserData(userRow);
            // Clean any leftover auth tokens/codes out of the URL once we\'re in
